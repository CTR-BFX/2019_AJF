...
## produce the heatmap with gene names and row clustering. 

subres5.ann <- subset(res5.ann, external_gene_name!="" & !is.na(padj))
select.no <- sum(abs(subres5.ann$log2FoldChange)>=3 & subres5.ann$padj < 0.05)
houtfile3 <- paste0(out.dir, "/", Project, "-Heatmap_sortData_padj0.05_l2fc3_topGenes41_rm27_rm3_23_07_2019.RData")

datasort <- heatmap_data_fn(subres5.ann, psig.level=0.05, lfc.level=3, select.no, houtfile3)
...
sampleTable.collapsed.rm$sample_id <- paste0(sampleTable.collapsed.rm$condition, "_", sampleTable.collapsed.rm$GestationalAge,
                                             "_", sampleTable.collapsed.rm$Labels)
# check gestational age as a factor
outHfile3 <- paste0(out.dir, "/", Project, "-Heatmap_TX_SHAM_padj0.05_lfc3_topGenes41_rm27_rm3_rmLC_23_07_2019.pdf")
rlds <- vsd.collapsed.rm
genes2plot <- s12.sig
ensE <- ensEMBL2id
samTable <- sampleTable.collapsed.rm
outfile <- outHfile3
width <- 7; height = 10
heatmap_mat_fn_age_GeneNames(rlds, genes2plot, ensE, samTable, outfile, width, height)
...
KEGG.TS.sel <- KEGG.sheep.TS_molten[selindex.new,]
## selected BP
selindexb <- c(4,11,12,16,21,23,22,24)
selindexb.new <- c(selindexb, selindexb+24)
BP.TS.sel <- BP.sheep.TS_molten[selindexb.new,]
BP.TS.sel.com <- BP.TS.sel[c(1:6,9:14),]

ss <- write.csv(BP.TS.sel, file = paste0(out.dir, "/", Project, "-BPpathway_sel8_ChangeCapital_Des_13_09_2019.csv"))
## edited in spreadsheet to change the first alphabet to capital one!
BP.TS.sel.new <- read.csv(paste0(out.dir, "/", Project, "-BPpathway_sel8_ChangeCapital_Des_13_09_2019.csv"))
BP.TS.sel.com$Description <- BP.TS.sel.new[,2]


barplotSum.kegg <- ggplot(KEGG.TS.sel, aes(x = reorder(Description, -qvalue), y = value, fill = variable)) +   # Fill column
  geom_bar(stat = "identity", width = 0.8) +   # draw the bars
  scale_y_continuous(breaks = brks,   # Breaks
                     labels = lbls,
                     limits=c(-25,20)
  ...
  theme_update(axis.title.x=element_blank(),
               ...
               axis.text.y= element_text(size=18, face="bold", hjust=0.95)) +
  theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
        axis.line = element_line(colour = "black"),panel.background = element_rect(fill = "white", colour = NA)) 


barplotSum.BP <- ggplot(BP.TS.sel.com, aes(x = reorder(Description, -qvalue), y = value, fill = variable)) +   # Fill column
  geom_bar(stat = "identity", width = 0.8) +   # draw the bars
  scale_y_continuous(breaks = brks,   # Breaks
                     labels = lbls,
                     limits=c(-25,20),
                     name="Gene Counts") + # Labels
  ...
  ylab("GeneCounts") +
  guides(fill=FALSE) +
  theme(plot.title = element_text(hjust = .2)) +   # Centre plot title
  scale_fill_manual(values = c("red", "blue"))+  # Color palette
  theme_bw() +
  theme_update(axis.title.x = element_text(size=18, face= "bold"),
